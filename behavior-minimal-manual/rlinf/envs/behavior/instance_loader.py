...
class ActivityInstanceLoader:
    ...
    def prepare_initial_env_config(self) -> None:
        """Adjust the initial OmniGibson config before env construction.

        Official BEHAVIOR tro_state caches are reset-time deltas. The initial
        OmniGibson environment still needs a full template file to exist; for the
        challenge cache on this machine that base template is instance 0. Keep
        ``self.activity_instance_id`` unchanged so reset can apply the requested
        cached tro_state afterward.
        """
        if not self.activity_instances:
            return
        file_format = self.activity_instances[0].file_format
        if file_format != "tro_state":
            return
        OmegaConf.update(self.omni_cfg, "task.activity_instance_id", 0)
