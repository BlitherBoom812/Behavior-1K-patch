...
from rlinf.envs.behavior.anti_stuck import AntiStuckWrapper
...
class BehaviorProcess:
    ...
    def __init__(self, cfg: DictConfig, conn, num_envs: int):
        self.conn = conn
        if os.environ.get("RLINF_DEBUG_WORKER_ENV") == "1":
            env_keys = (
                "CUDA_VISIBLE_DEVICES",
                "VISIBLE_DEVICES",
                "RLINF_PLACEMENT_CUDA_VISIBLE_DEVICES",
            ...
            print(f"[RLINF_BEHAVIOR_SUBPROC_ENV] {env_snapshot}", flush=True)
        from omnigibson.envs import VectorEnvironment

        omni_cfg = setup_omni_cfg(cfg)
        self.instance_loader = ActivityInstanceLoader.from_omni_cfg(omni_cfg)
        self.instance_loader.prepare_initial_env_config()
        ...
        self.anti_stuck = [
            AntiStuckWrapper.from_env(
                self.instance_loader.activity_name,
                log_prefix=f"env={env_idx}",
            )
            for env_idx in range(num_envs)
        ]
        self.third_person_video_enabled = os.environ.get(
            "ANTI_STUCK_THIRD_PERSON_VIDEO_ENABLE", "0"
        ).lower() in {"1", "true", "yes"}
        self._third_person_camera_missing_logged = False
    ...
    def _robot_for_env(self, env_idx: int):
        env = self._child_env(self.env.envs[env_idx])
        return env.robots[0]

    def _task_scope_for_env(self, env_idx: int):
        env = self._child_env(self.env.envs[env_idx])
        task = getattr(env, "task", getattr(env, "_task", None))
        return getattr(task, "object_scope", None)
    ...
    def step_env(self, actions, need_obs: bool):
        ...
        if not need_obs:
            ...
            raw_obs = None
        self._add_anti_stuck_info(infos, capture_third_person=need_obs)
        return (
            raw_obs,
    ...
    def reset(self, payload):
        ...
        for wrapper in self.anti_stuck:
            wrapper.reset()
        self.conn.send({"result": (raw_obs, infos)})

    def _apply_anti_stuck(self, actions):
        if not self.anti_stuck or not any(wrapper.enabled for wrapper in self.anti_stuck):
            return actions

        adjusted_actions = actions.clone() if isinstance(actions, torch.Tensor) else torch.as_tensor(actions).clone()
        for env_idx, wrapper in enumerate(self.anti_stuck):
            wrapper.tick_env_step()
            policy_action = adjusted_actions[env_idx]
            robot = self._robot_for_env(env_idx)
            task_scope = self._task_scope_for_env(env_idx)
            if wrapper.is_escaping():
                if wrapper.suppress_for_task_progress(robot, task_scope, abort_escape=True):
                    adjusted_actions[env_idx] = policy_action
                else:
                    adjusted_actions[env_idx] = wrapper.step(policy_action, robot)
            elif wrapper.update_and_check_stuck(robot, policy_action, task_scope):
                wrapper.start_escape(robot)
                adjusted_actions[env_idx] = wrapper.step(policy_action, robot)
        return adjusted_actions

    def _consume_anti_stuck_finished(self):
        for env_idx, wrapper in enumerate(self.anti_stuck):
            if wrapper.consume_finished_escape(self._robot_for_env(env_idx)):
                print(
                    "[AntiStuck] env=%d RLinf chunk queue note: no websocket policy queue reset is needed" % env_idx,
                    flush=True,
                )

    def _add_anti_stuck_info(self, infos, capture_third_person: bool = False):
        if not self.anti_stuck or not isinstance(infos, list):
            return
        for env_idx, wrapper in enumerate(self.anti_stuck):
            if env_idx >= len(infos) or not isinstance(infos[env_idx], dict):
                continue
            robot = self._robot_for_env(env_idx)
            base_pos, base_orn = wrapper.robot_pose(robot)
            roll, pitch, _yaw = wrapper.robot_rpy(robot)
            infos[env_idx]["anti_stuck"] = {
                "enabled": bool(wrapper.enabled),
                "version": wrapper.version,
                "env_step": int(wrapper.env_step_counter),
                "check_step": int(wrapper.step_counter),
                "base_height": float(base_pos[2]),
                "base_roll": float(roll),
                "base_pitch": float(pitch),
                "label": getattr(wrapper, "video_label", ""),
                "suppress_label": getattr(wrapper, "suppress_video_label", ""),
                "is_escaping": bool(wrapper.is_escaping()),
            }
            if self.third_person_video_enabled and capture_third_person:
                third_person = self._capture_third_person_frame(robot)
                if third_person is not None:
                    infos[env_idx]["third_person_image"] = third_person

    def chunk_step(self, chunk_actions):
        chunk_size = chunk_actions.shape[1]
        results = []
        for i in range(chunk_size):
            actions = self._apply_anti_stuck(chunk_actions[:, i])
            is_last = i == chunk_size - 1
            need_obs = not self.skip_intermediate_obs_in_chunk or is_last
            step_result = self.step_env(actions, need_obs=need_obs)
            self._consume_anti_stuck_finished()
            results.append(step_result)
        results = tuple(zip(*results))
        self.conn.send({"result": results})

    @staticmethod
    def _look_at_quat(camera_pos: torch.Tensor, target_pos: torch.Tensor) -> torch.Tensor:
        from omnigibson.utils import transform_utils as T

        forward = target_pos - camera_pos
        forward = forward / torch.clamp(torch.linalg.norm(forward), min=1e-6)
        world_up = torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device=forward.device)
        right = torch.linalg.cross(forward, world_up)
        if torch.linalg.norm(right) < 1e-6:
            right = torch.tensor([1.0, 0.0, 0.0], dtype=torch.float32, device=forward.device)
        right = right / torch.clamp(torch.linalg.norm(right), min=1e-6)
        up = torch.linalg.cross(right, forward)
        up = up / torch.clamp(torch.linalg.norm(up), min=1e-6)
        # OmniGibson cameras look along local -Z, with local Y as up.
        rot = torch.stack([right, up, -forward], dim=1)
        return T.mat2quat(rot)

    def _capture_third_person_frame(self, robot):
        try:
            import omnigibson as og

            base_pos, base_orn = robot.get_position_orientation()
            if isinstance(base_pos, torch.Tensor):
                base_pos = base_pos.detach().cpu().to(dtype=torch.float32)
            else:
                base_pos = torch.as_tensor(base_pos, dtype=torch.float32)
            if isinstance(base_orn, torch.Tensor):
                base_orn = base_orn.detach().cpu().to(dtype=torch.float32)
            else:
                base_orn = torch.as_tensor(base_orn, dtype=torch.float32)
            from omnigibson.utils import transform_utils as T

            rot = T.quat2mat(base_orn)
            backward = -rot[:, 0]
            lateral = rot[:, 1] * 0.8
            camera_pos = base_pos + backward * 3.0 + lateral + torch.tensor([0.0, 0.0, 1.8])
            target_pos = base_pos + torch.tensor([0.0, 0.0, 0.75])
            camera_orn = self._look_at_quat(camera_pos, target_pos)
            viewer_camera = getattr(og.sim, "viewer_camera", None)
            if viewer_camera is None:
                if not self._third_person_camera_missing_logged:
                    print("[AntiStuck] third-person capture skipped: viewer_camera is not enabled", flush=True)
                    self._third_person_camera_missing_logged = True
                return None
            viewer_camera.set_position_orientation(position=camera_pos, orientation=camera_orn)
            obs, _ = viewer_camera.get_obs()
            image = obs.get("rgb")
            if image is None:
                return None
            ...
            print(f"[AntiStuck] third-person capture failed: {exc}", flush=True)
            return None
...
class BehaviorEnv(gym.Env):
    ...
    def _record_metrics(self, rewards, infos):
        ...
        for env_idx, (reward, info) in enumerate(zip(rewards, infos)):
            ...
            anti_stuck_infos.append(info.get("anti_stuck", {}))

        infos = {
            "episode": to_tensor(list_of_dict_to_dict_of_list(info_lists)),
            "anti_stuck": anti_stuck_infos,
        }
        return infos
