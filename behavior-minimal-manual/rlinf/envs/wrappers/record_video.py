...
class RecordVideo(gym.Wrapper):
    ...
    def __init__(self, env: gym.Env, video_cfg, fps: Optional[int] = None):
        ...
        self.third_person_images: list[np.ndarray] = []
        self.video_cnt = 0
        ...
        self._slow_head_enabled = os.environ.get(
            "ANTI_STUCK_SLOW_HEAD_ENABLE",
            "0",
        ).lower() in {"1", "true", "yes"}
        self._slow_head_factor = max(
            1.0, float(os.environ.get("ANTI_STUCK_SLOW_HEAD_FACTOR", "5.0"))
        )
    ...
    def _info_for_env(self, infos: Any, env_id: int) -> Any:
    ...
    def _anti_stuck_info_for_env(self, infos: Any, env_id: int) -> dict:
        if isinstance(infos, dict) and "anti_stuck" in infos:
            anti_stuck = infos.get("anti_stuck")
            if isinstance(anti_stuck, dict):
                return anti_stuck
            if isinstance(anti_stuck, (list, tuple)):
                if len(anti_stuck) > env_id and isinstance(anti_stuck[env_id], dict):
                    return anti_stuck[env_id]
                if anti_stuck and isinstance(anti_stuck[0], dict):
                    return anti_stuck[0]
            return {}
        info = self._info_for_env(infos, env_id)
        if not isinstance(info, dict):
            return {}
        anti_stuck = info.get("anti_stuck")
        return anti_stuck if isinstance(anti_stuck, dict) else {}

    def _third_person_image_for_env(self, infos: Any, env_id: int) -> Optional[np.ndarray]:
        info = self._info_for_env(infos, env_id)
        if not isinstance(info, dict) or "third_person_image" not in info:
            return None
        image = self._to_numpy(info["third_person_image"])
        if image.ndim < 3:
            return None
    ...
    def _draw_anti_stuck_overlay(
        ...
        version = anti_stuck.get("version", "unversioned")
        env_step = anti_stuck.get("env_step", -1)
        check_step = anti_stuck.get("check_step", -1)
        base_height = anti_stuck.get("base_height", None)
        base_roll = anti_stuck.get("base_roll", None)
        base_pitch = anti_stuck.get("base_pitch", None)
        label = anti_stuck.get("label", "") or ""
        suppress_label = anti_stuck.get("suppress_label", "") or ""
        pose_label = ""
        if base_height is not None and base_roll is not None and base_pitch is not None:
            pose_label = f" h={float(base_height):.2f} r={float(base_roll):.2f} p={float(base_pitch):.2f}"
        status_label = f"{version} env={env_id} env_step={env_step} check_step={check_step} frame={frame_idx}{pose_label}"
        labels = [status_label]
        if suppress_label:
            labels.append(str(suppress_label))
        if label:
            labels.append(str(label))

        h, w = frame.shape[:2]
        has_alpha = frame.shape[-1] == 4
        image = Image.fromarray(frame)
        draw = ImageDraw.Draw(image, "RGBA")
        font = ImageFont.load_default()
        line_h = max(14, min(26, h // 8))
        bar_h = min(h, line_h * len(labels) + 6)
        colors = {
            "status": (24, 28, 34, 230),
            "suppress": (178, 106, 0, 235),
            "antistuck": (210, 36, 28, 235),
        }
        draw.rectangle((0, 0, w, min(line_h + 3, bar_h)), fill=colors["status"])
        row = 1
        if suppress_label:
            y0 = line_h * row + 3
            draw.rectangle((0, y0, w, min(line_h * (row + 1) + 3, bar_h)), fill=colors["suppress"])
            row += 1
        if label:
            y0 = line_h * row + 3
            draw.rectangle((0, y0, w, bar_h), fill=colors["antistuck"])

        for i, text in enumerate(labels):
            text = str(text)
            max_chars = max(12, int(w / 6))
            if len(text) > max_chars:
                text = text[: max_chars - 3] + "..."
            y = 4 + line_h * i
            draw.text((4, y), text, fill=(255, 255, 255, 255), font=font)

        out = np.asarray(image)
        if not has_alpha and out.shape[-1] == 4:
            out = out[:, :, :3]
    ...
    def _append_frame(
        ...
        frame_idx = len(self.render_images) + 1
        images = [
            self._draw_anti_stuck_overlay(
                img,
                self._anti_stuck_info_for_env(infos, env_id),
                env_id,
                frame_idx,
            )
            for env_id, img in enumerate(images)
        ...
        third_person_images = []
        for env_id in range(len(images)):
            third_person = self._third_person_image_for_env(infos, env_id)
            if third_person is None:
                continue
            third_person_images.append(
                self._draw_anti_stuck_overlay(
                    third_person,
                    self._anti_stuck_info_for_env(infos, env_id),
                    env_id,
        ...
        if third_person_images:
            if len(third_person_images) > 1:
                nrows = int(np.sqrt(len(third_person_images)))
                self.third_person_images.append(tile_images(third_person_images, nrows=nrows))
            else:
                self.third_person_images.append(third_person_images[0])
    ...
    def flush_video(self, video_sub_dir: Optional[str] = None):
        ...
        if not self.render_images and not self.third_person_images:
            return
        ...
        mp4_path = os.path.join(output_dir, f"{video_idx}.mp4")
        frames = list(self.render_images)
        third_person_frames = list(self.third_person_images)
        self.render_images = []
        self.third_person_images = []
        self.video_cnt += 1
        if frames:
            self._submit_save(frames, mp4_path)
        if third_person_frames:
            third_person_name = (
                "third_person.mp4" if video_idx == 0 else f"third_person_{video_idx}.mp4"
            )
            self._submit_save(third_person_frames, os.path.join(output_dir, third_person_name))
        if self._slow_head_enabled:
            slow_name = (
                f"head_slow_{self._slow_head_factor:g}x.mp4"
                if video_idx == 0
                else f"head_slow_{self._slow_head_factor:g}x_{video_idx}.mp4"
            )
            slow_path = os.path.join(output_dir, slow_name)
            self._submit_slow_copy(mp4_path, slow_path)
        if os.environ.get("RLINF_VIDEO_SYNC_FLUSH", "0").lower() in {"1", "true", "yes"}:
    ...
    def _submit_save(
        ...
        self._prune_futures()
        future = self._executor.submit(self._save_video, frames, mp4_path, fps)
    ...
    def _save_video(
        ...
        video_writer = None
        try:
            video_writer = imageio.get_writer(mp4_path, fps=fps or self._fps)
            for img in frames:
    ...
    def _submit_slow_copy(self, source_path: str, slow_path: str) -> None:
        self._prune_futures()
        future = self._executor.submit(self._save_slow_copy, source_path, slow_path)
        self._save_futures.append(future)

    def _save_slow_copy(self, source_path: str, slow_path: str) -> None:
        """Create a browser-compatible slow copy after the normal MP4 is saved."""
        ffmpeg_exe = (
        ...
        cmd = [
            ...
            source_path,
            "-vf",
            f"setpts={self._slow_head_factor:g}*PTS",
            "-an",
            "-c:v",
            "libx264",
            "-preset",
        ...
        try:
            ...
            warnings.warn(f"Failed to save slow video {slow_path}: {exc}")
    ...
    def _wait_for_saves(self) -> None:
        """Block until all submitted video writes finish."""
        futures = list(self._save_futures)
        for future in futures:
            try:
                future.result()
            except Exception as exc:
                warnings.warn(f"Video save worker failed: {exc}")
        self._prune_futures()
