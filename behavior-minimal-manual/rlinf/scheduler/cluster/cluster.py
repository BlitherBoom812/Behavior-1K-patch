...
class Cluster:
    ...
    @classmethod
    def _sync_namespace_from_env(cls) -> None:
        namespace = os.environ.get("CLUSTER_NAMESPACE")
        if namespace:
            cls.NAMESPACE = namespace

    @classmethod
    def ray_init_kwargs(cls, connect_existing: bool = True) -> dict[str, Any]:
        """Build Ray init kwargs without accidentally attaching to another local job."""
        cls._sync_namespace_from_env()
        kwargs: dict[str, Any] = {
            "logging_level": cls.LOGGING_LEVEL,
            "namespace": cls.NAMESPACE,
        }

        explicit_address = os.environ.get("RAY_ADDRESS") or os.environ.get(
            "RLINF_RAY_ADDRESS"
        )
        isolated = cls._env_truthy("RLINF_RAY_ISOLATED")
        if explicit_address:
            kwargs["address"] = explicit_address
        elif connect_existing and not isolated:
            kwargs["address"] = "auto"

        temp_dir = os.environ.get("RLINF_RAY_TEMP_DIR")
        if temp_dir and "address" not in kwargs:
            kwargs["_temp_dir"] = temp_dir

        return kwargs

    @staticmethod
    def publish_ray_address_to_env() -> None:
        """Expose the current private Ray address to subprocesses launched later."""
    ...
    def __init__(
        ...
        if self._has_initialized:
            return
        Cluster._sync_namespace_from_env()
        self._setup_logger()
    ...
    def _init_and_launch_managers(
        ...
        try:
            ...
            ray_init_kwargs = Cluster.ray_init_kwargs(connect_existing=True)
            ...
            ray_init_kwargs = Cluster.ray_init_kwargs(connect_existing=False)
            if self._ray_code_sync_fragment is not None:
                ray_init_kwargs["runtime_env"] = dict(self._ray_code_sync_fragment)
            ray.init(**ray_init_kwargs)
        Cluster.publish_ray_address_to_env()
    ...
    def _init_from_existing_managers(self):
        if not ray.is_initialized():
            ray.init(**Cluster.ray_init_kwargs(connect_existing=True))
            Cluster.publish_ray_address_to_env()
