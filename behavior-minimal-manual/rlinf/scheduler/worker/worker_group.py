...
class WorkerGroup(Generic[WorkerClsType]):
    ...
    def _create_workers(self):
        ...
        for placement in placements:
            ...
            preserve_groups = {
                group_name.strip()
                for group_name in os.environ.get(
                    "RLINF_PRESERVE_CUDA_VISIBLE_DEVICES_GROUPS", ""
                ).split(",")
                if group_name.strip()
            }
            if self._worker_group_name in preserve_groups:
                parent_cuda_visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES")
                if parent_cuda_visible_devices:
                    env_vars["CUDA_VISIBLE_DEVICES"] = parent_cuda_visible_devices
                    env_vars["VISIBLE_DEVICES"] = parent_cuda_visible_devices
                    env_vars["RLINF_PLACEMENT_CUDA_VISIBLE_DEVICES"] = ",".join(
                        placement.visible_accelerators
                    )
            if os.environ.get("RLINF_DEBUG_WORKER_ENV") == "1":
                print(
                    "[RLINF_WORKER_ENV] "
                    f"group={self._worker_group_name} rank={placement.rank} "
                    f"placement={','.join(placement.visible_accelerators)} "
                    f"CUDA_VISIBLE_DEVICES={env_vars.get('CUDA_VISIBLE_DEVICES')} "
                    f"VISIBLE_DEVICES={env_vars.get('VISIBLE_DEVICES')} "
                    f"RLINF_PLACEMENT_CUDA_VISIBLE_DEVICES="
                    f"{env_vars.get('RLINF_PLACEMENT_CUDA_VISIBLE_DEVICES')}",
                    flush=True,
                )
