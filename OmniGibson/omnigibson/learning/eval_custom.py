...
if __name__ == "__main__":
    ...
    with Evaluator(config) as evaluator:
        ...
        num_eval_episodes = int(OmegaConf.select(config, "num_eval_episodes", default=m.NUM_EVAL_EPISODES))
        eval_episode_start = int(OmegaConf.select(config, "eval_episode_start", default=0))
        skip_existing_metrics = bool(OmegaConf.select(config, "skip_existing_metrics", default=False))
        for epi in range(eval_episode_start, eval_episode_start + num_eval_episodes):
            evaluator.reset()
            for idx in instances_to_run:
                metric_path = metrics_path / f"{config.task.name}_{idx}_{epi}.json"
                if skip_existing_metrics and metric_path.exists():
                    logger.info(f"Skipping existing task instance {idx} / episode {epi}: {metric_path}")
                    continue
                evaluator.reset()
                ...
                episode_success = bool(info["done"]["success"])
                ...
                logger.info(f"Evaluation success: {episode_success}")
                ...
                metrics = {
                    ...
                    "num_steps": int(evaluator.env._current_step),
                    "perturb_pose": bool(config.perturb_pose),
                }
                for metric in evaluator.metrics:
